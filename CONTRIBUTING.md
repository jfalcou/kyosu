# Contributing to KYOSU

Thank you for your interest in KYOSU. Bug reports, documentation fixes, a missing overload on one
algebra and whole new functions are all welcome.
...
merged pull requests. **You are welcome.** A student who wants to see how quaternions and octonions
are computed with is exactly who we wrote this for, and nobody is going to hold a deadline against
you.
...
issue](https://github.com/jfalcou/kyosu/labels/good%20first%20issue) label lists work that is
...
```bash
cmake --preset gcc
cmake --build build/gcc --target unit.exp.exe
ctest --preset gcc
```

One target per test file, and CMake globs the test directories, so a new `.cpp` needs no CMake edit.
KYOSU pulls EVE, which is heavy: build the one target you are working on rather than the suite.
...
- **the documentation of what you touched**, with the `kyosu-doxygen` target still silent, since a
...
algebra, and the input values. A Godbolt link that reproduces it replaces all three.

**Work on an open issue.** The [open issues](https://github.com/jfalcou/kyosu/issues) are all fair
...
cmake --build build/gcc --target kyosu-doxygen
```
