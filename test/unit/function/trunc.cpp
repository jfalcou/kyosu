...
TTS_CASE_WITH("Check kyosu::trunc over real", kyosu::real_types, tts::randoms(-10, 10))
(auto data)
{
  TTS_EQUAL(kyosu::trunc(data), eve::trunc(data));
};

TTS_CASE_WITH("Check kyosu::trunc over complex", kyosu::real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
(auto r, auto i)
{
...
<typename T>(T r, T i, T j, T k, T l, T li, T lj, T lk)
{
  ...
  TTS_EQUAL(kyosu::trunc(type(r, i, j, k, l, li, lj, lk)),
            type(eve::trunc(r), eve::trunc(i), eve::trunc(j), eve::trunc(k), eve::trunc(l), eve::trunc(li),
                 eve::trunc(lj), eve::trunc(lk)));
};
