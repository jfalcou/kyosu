...
TTS_CASE_TPL("Check hyper 1F0", kyosu::scalar_real_types)
<typename T>(tts::type<T>)
{
  auto pr = tts::prec<T>(4.0e-3, 1.0e-8);
  using r_t = kyosu::cayley_dickson<T, 2>;
  r_t res[] = {r_t(1.0, 0.0), r_t(0.8227512861083565818, 0), kyosu::cinf(eve::as<r_t>()),
               r_t(6.822479299281938112227, 0.0), r_t(3.825376991857549983, -1.9848051342921483)};
  int i = 0;
