...
TTS_CASE_WITH("Check kyosu::nearest over real", kyosu::real_types, tts::randoms(-10, 10))
(auto data)
{
  TTS_EQUAL(kyosu::nearest(data), eve::nearest(data));
};

TTS_CASE_WITH("Check kyosu::nearest over complex", kyosu::real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
(auto r, auto i)
{
...
<typename T>(T r, T i, T j, T k, T l, T li, T lj, T lk)
{
  ...
  TTS_EQUAL(kyosu::nearest(type(r, i, j, k, l, li, lj, lk)),
            type(eve::nearest(r), eve::nearest(i), eve::nearest(j), eve::nearest(k), eve::nearest(l), eve::nearest(li),
                 eve::nearest(lj), eve::nearest(lk)));
};
