...
{
  auto prec = (sizeof(eve::element_type_t<decltype(r0)>) == 4) ? 1.0e-3 : 1.0e-6;
  auto c0 = kyosu::complex(r0,i0);
  auto c1 = kyosu::complex(r1,i1);
  TTS_RELATIVE_EQUAL(kyosu::lpnorm(p, c0, c1), eve::lpnorm(p, kyosu::abs(c0), kyosu::abs(c1)), prec);
  TTS_RELATIVE_EQUAL(kyosu::lpnorm(2, c0, c1), kyosu::hypot(c0, c1) , prec);
};
...
<typename T>(T r0, T i0, T j0, T k0, T r1, T i1, T j1, T k1, T p)
{
  ...
  TTS_RELATIVE_EQUAL(kyosu::lpnorm(p, q0, q1), eve::lpnorm(p, kyosu::abs(q0), kyosu::abs(q1)), prec);
};
