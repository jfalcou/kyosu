  ...
  <typename T>(T )
{
  if constexpr(sizeof(T) == 8)
  {
    ...
    for (int k = 0; k < 16; ++k)
    {
      ...
      auto yl = kyosu::cyl_bessel_y(v1, c, std::span(is_03));
      auto myl = kyosu::bessel_y(v1, c, std::span(mis_03));
      TTS_RELATIVE_EQUAL(myl, yl, 1.e-2);
//      auto vv = T(N+1.0/3);
      for (n = 0; n <= N; ++n)
      {
//        auto vn = v1+n;
...
//        TTS_RELATIVE_EQUAL(refi_03,  kyosu::cyl_bessel_y(vn, c), 1.e-2) << "n " << n  << " k " << k  << " c "<< c <<'\n';
  ...
  <typename T>(T )
{
  if constexpr(sizeof(T) == 8)
  {
    ...
    std::vector<kyosu::complex_t<T>> is_03(N+2), mis_03(N+2);
    for (int k = 0; k < 16; ++k)
    {
      auto c = kyosu::complex(re[k], im[k]);
      kyosu::cyl_bessel_y(v1+N, c, std::span(is_03));
      kyosu::bessel_y(v1+N, c, std::span(mis_03));
//      auto vv = -T(N+1.0/3);
      for (n = 0; n <= N; ++n)
      {
        auto vn = v1-n;
        auto refi_03=  kyosu::complex_t<T>(reresi_03[n][k], imresi_03[n][k]);
        std::cout << n << " -> " << refi_03 << std::endl;
        TTS_RELATIVE_EQUAL(refi_03, is_03[n], 1.e-2) << "n " << n  << " k " << k  << " c "<< c << " arg(c) "<< kyosu::arg(kyosu::conj(c)) << " < 1.57" << '\n';
        TTS_RELATIVE_EQUAL(kyosu::cyl_bessel_y(vn, c), is_03[n], 1.e-2);
        if (n <= N) TTS_RELATIVE_EQUAL(mis_03[n], is_03[n], 1.e-2);
      }
    }
  }
