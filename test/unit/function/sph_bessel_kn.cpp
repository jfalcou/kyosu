...
TTS_CASE_WITH ( "Check kyosu::sph_bessel_in over real"
...
<typename T>(T )
{
  if constexpr(sizeof(T) == 8)
  {
    ...
    for(size_t i=0; i < re.size(); ++i)
    {
      ...
      TTS_RELATIVE_EQUAL(kyosu::sph_bessel_kn(3, c), res, 1.0e-6) << i <<  " <- " << c << '\n';
    }
  }
};
...
//  using u_t = eve::underlying_type_t<T>;
 ...
 for(int i=0; i < 10; ++i)
 {
  auto knc = sph_bessel_kn(i, c);
  TTS_IEEE_EQUAL(knc, kyosu::conj(sph_bessel_kn(i, cb)));
  TTS_EXPECT(eve::all(kyosu::is_real(cr)));
  TTS_EXPECT(eve::all(kyosu::is_pure(ci)));
  TTS_IEEE_EQUAL(sph_bessel_kn(i, zer), inf) << i << '\n';
  TTS_IEEE_EQUAL(knc, sph_bessel_kn(-i, c));
 }
};
