...
TTS_CASE_WITH("Check kyosu::cyl_bessel_j0 over real", kyosu::scalar_real_types, tts::randoms(-10, 10))
<typename T>(T)
{
  ...
  if constexpr (sizeof(e_t) == 8)
  {
    ...
    std::array<T, 16> reres{
      -5.7419608040318385e+07, -1.2409242416987633e+06, -2.6427659633595144e+04, -3.4216029692014837e+02,
      3.0804691728218572e+01,  6.6903996777286583e+00,  1.3523669469741970e+00,  5.5870307892357129e-01,
      7.5248558508731545e-01,  7.1665807338276855e-01,  -7.3369695701857451e-01, -1.3293492209488911e-01,
      1.1081359835209212e+00,  -2.0805776844472024e+00, 3.1132412017028535e+00,  -3.5952187284708805e+00};

    std::array<T, 16> imres{
      1.6763717755961375e+07,  -1.0671669307691315e+06, 5.1770135827841987e+04,  -2.7209127089468357e+03,
      1.7464925781992241e+02,  -1.5040759898113270e+01, 1.9670200948712224e+00,  -4.4241768244532709e-01,
      0.0000000000000000e+00,  6.1988929040084473e-01,  -4.7545485913890873e-01, -1.2208933006131721e+00,
      -1.3901622231770903e+00, -1.5528897868070297e+00, -2.4569086817625085e+00, -5.1871698800009600e+00};

    for (int i = 0; i < 16; ++i)
    {
      auto c = kyosu::complex(re[i], im[i]);
      auto res = kyosu::complex(reres[i], imres[i]);
      TTS_RELATIVE_EQUAL(kyosu::airy_bi(c), res, tts::prec<T>()) << i << " <- " << c << '\n';
...
TTS_CASE_WITH("Check kyosu::airy_bi", kyosu::real_types, tts::randoms(-10, 10), tts::randoms(-10, 10)
