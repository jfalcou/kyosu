...
TTS_CASE_WITH("Check kyosu::nthroot over real", kyosu::simd_real_types, tts::randoms(-10, 10))
<typename T>(T r0)
{
  // primary root
  ...
  // simd k simd n
  TTS_RELATIVE_EQUAL(kyosu::nthroot(r0, e_t(3), fn), kyosu::nthroot(r0, e_t(3)) * kyosu::exp_ipi(fn * 2 / 3),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(r0, e_t(3), n), kyosu::nthroot(r0, e_t(3)) * kyosu::exp_ipi(fn * 2 / 3),
                     tts::prec<T>());

  // scalar k simd n
  TTS_RELATIVE_EQUAL(kyosu::nthroot(r0, fn, e_t(3)), kyosu::nthroot(r0, fn) * kyosu::exp_ipi(e_t(6) / fn),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(r0, n, e_t(3)), kyosu::nthroot(r0, n) * kyosu::exp_ipi(e_t(6) / fn),
                     tts::prec<T>());
...
TTS_CASE_WITH("Check kyosu::nthroot over complex", kyosu::simd_real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
<typename T>(T r0, T i0)
{
  ...
  // scalar k scalar n
  using e_t = eve::element_type_t<T>;
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, e_t(3), e_t(2)), kyosu::nthroot(c0, e_t(3)) * kyosu::exp_ipi(e_t(4) / 3),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, e_t(3), 2), kyosu::nthroot(c0, e_t(3)) * kyosu::exp_ipi(e_t(4) / 3),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, 3, 2), kyosu::nthroot(c0, e_t(3)) * kyosu::exp_ipi(e_t(4) / 3), tts::prec<T>());

  // simd k simd n
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, e_t(3), fn), kyosu::nthroot(c0, e_t(3)) * kyosu::exp_ipi(fn * 2 / 3),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, e_t(3), n), kyosu::nthroot(c0, e_t(3)) * kyosu::exp_ipi(fn * 2 / 3),
                     tts::prec<T>());

  // scalar k simd n
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, fn, e_t(3)), kyosu::nthroot(c0, fn) * kyosu::exp_ipi(e_t(6) / fn),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::nthroot(c0, n, e_t(3)), kyosu::nthroot(c0, n) * kyosu::exp_ipi(e_t(6) / fn),
                     tts::prec<T>());
