...
TTS_CASE_WITH("Check kyosu::ceil over real", kyosu::real_types, tts::randoms(-10, 10))
(auto data)
{
  TTS_EQUAL(kyosu::ceil(data), eve::ceil(data));
};

TTS_CASE_WITH("Check kyosu::ceil over complex", kyosu::real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
(auto r, auto i)
{
...
<typename T>(T r, T i, T j, T k, T l, T li, T lj, T lk)
{
  ...
  TTS_EQUAL(kyosu::ceil(type(r, i, j, k, l, li, lj, lk)),
            type(eve::ceil(r), eve::ceil(i), eve::ceil(j), eve::ceil(k), eve::ceil(l), eve::ceil(li), eve::ceil(lj),
                 eve::ceil(lk)));
};
