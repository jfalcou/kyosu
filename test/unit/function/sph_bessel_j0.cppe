...
TTS_CASE_WITH ( "Check kyosu::cyl_lbessel_j0 over real"
...
<typename T>(T )
{
  ...
  if constexpr(sizeof(e_t) == 8)
  {
    ...
    for(int i=0; i < 10; ++i)
    {
      ...
      TTS_RELATIVE_EQUAL(kyosu::cyl_lbessel_j0(c), res, 1.0e-4) << i <<  " <- " << c << '\n';
    }
  }
  std::cout.precision(17);
  ...
  for(int i=0; i < 8; ++i)
  {
   ...
   TTS_RELATIVE_EQUAL(kyosu::cyl_lbessel_j0(c), res, tol) << i <<  " <- " << c << '\n';
  }
};
...
<typename T>(T a0, T a1)
{
  if constexpr(sizeof(T) == 8)
  {
    ...
    auto cm=  -c;
    using kyosu::cyl_lbessel_j0;
    using kyosu::cyl_bessel_j0;
    auto lj0c = cyl_lbessel_j0(c);
    auto logj0c = kyosu::log(cyl_bessel_j0(c));
    TTS_RELATIVE_EQUAL(lj0c, logj0c, tol) << "c " << c << "\n";

    auto lj0cb = cyl_lbessel_j0(cb);
    auto logj0cb = kyosu::log(cyl_bessel_j0(cb));
    TTS_RELATIVE_EQUAL(lj0cb, logj0cb, tol) << "c " << c << "\n";

    auto lj0cm = cyl_lbessel_j0(cm);
    auto logj0cm = kyosu::log(cyl_bessel_j0(cm));
    TTS_RELATIVE_EQUAL(lj0cm, logj0cm, tol) << "c " << c << "\n";
    auto z =   kyosu::complex(T(0), T(0));
    TTS_IEEE_EQUAL(cyl_lbessel_j0(z), kyosu::complex(eve::zero(eve::as<e_t>())));
  }
};
