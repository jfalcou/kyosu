...
TTS_CASE_WITH("Check kyosu::is_real over real", kyosu::real_types, tts::randoms(-10, 10))
(auto data)
{
  TTS_EQUAL(kyosu::is_real(data), eve::true_(eve::as(data)));
};

TTS_CASE_WITH("Check kyosu::is_real over complex", kyosu::real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
(auto r, auto i)
{
...
<typename T>(T r, T i, T j, T k, T l, T li, T lj, T lk)
{
  ...
  TTS_EQUAL(kyosu::is_real(type(r, i, j, k, l, li, lj, lk)), eve::is_eqz(i) && eve::is_eqz(j) && eve::is_eqz(k) &&
                                                               eve::is_eqz(l) && eve::is_eqz(li) && eve::is_eqz(lj) &&
                                                               eve::is_eqz(lk));
  TTS_EQUAL(kyosu::is_real(type(r)), eve::true_(eve::as(r)));
};
