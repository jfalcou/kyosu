...
<typename T>(tts::type<T>)
{
  ...
  kyosu::quaternion<T>             z_sd{};
  kyosu::quaternion<T>             z_s0{spi};
  kyosu::quaternion<T>             z_ss{spi,so, so, so};
  eve::wide<kyosu::quaternion<T>>  z_vd{};
  eve::wide<kyosu::quaternion<T>>  z_vs{wo,spi,spi,spi};
  eve::wide<kyosu::quaternion<T>>  z_sv{spi,wo,wo,wo};
  eve::wide<kyosu::quaternion<T>>  z_vv{wo,wpi,wpi,wpi};

  // auto a = kumi::make_tuple(wpi,wpi,wpi);
  // eve::wide<kyosu::quaternion<T>> z_ra =  eve::to_quaternion(wo,a);
...
<typename T>(tts::type<T>)
{
  ...
  auto fill   = [](auto i, auto) { return kyosu::quaternion<T>(T(1+i),T(1)/(1+i),T(i),T(1)); };

  eve::wide<kyosu::quaternion<T>> z_l(fill);
  eve::wide<kyosu::quaternion<T>> z_ll(fill_r, fill_i, fill_j, fill_k);
  eve::wide<kyosu::quaternion<T>> z_lc(fill_r, T{1.25}, T{2.25}, T{3.25});
  eve::wide<kyosu::quaternion<T>> z_cl(T{8.8},fill_i, T(1.0), fill_k);
...
<typename T>(tts::type<T>)
{
  ...
  kyosu::quaternion<T>  z_s{spi,so, so, so};
  auto                  z_sc{z_s};
  kyosu::quaternion<T>  z_sa;
  z_sa = z_s;

  eve::wide<kyosu::quaternion<T>> z_v{wo,wpi, wpi, wpi};
  auto                            z_vc{z_v};
  eve::wide<kyosu::quaternion<T>> z_va;
  z_va = z_v;
