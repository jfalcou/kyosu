...
<typename T>(T const& a0, T const& a1)
{
  ...
  for (size_t i = 0; i < a0.size(); ++i)
  {
    ...
    TTS_RELATIVE_EQUAL(kyosu::sqrt(kc_t(e, f)), cv(std::sqrt(c_t(e, f))), tts::prec<T>());
  }
};

TTS_CASE_WITH("Check behavior of sqrt on wide", kyosu::simd_real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
<typename T>(T const& a0, T const& a1)
{
  using e_t = T;
  ...
  TTS_RELATIVE_EQUAL(kyosu::sqrt(ke_t{a0, a1}), e, tts::prec<T>());
};
