...
<typename T>(tts::type<T>)
{
  ...
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(1, 1)), tcx(-0.6509231993018563, -0.301640320467533197887531),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(-1, 1)), tcx(-0.9974967895818289, +2.054554169724811730959450),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(1, -1)), tcx(-0.6509231993018563, +0.301640320467533197887531),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(-1, -1)), tcx(-0.997496789581828993, -2.054554169724811730959450),
                     tts::prec<T>());
  TTS_IEEE_EQUAL(kyosu::log_gamma(tcx(-1, 0)), tcx(eve::nan(eve::as<e_t>()), eve::inf(eve::as<e_t>())));
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(1, -2)), tcx(-1.876078786430929341, -0.12964631630978831138370),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::log_gamma(tcx(-1, -2)), tcx(-3.37394492320792483, -2.80759136089877543363209),
                     tts::prec<T>());
};
