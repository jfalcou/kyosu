...
<typename T>(tts::type<T>)
{
  ...
  TTS_RELATIVE_EQUAL(kyosu::beta(i, i), z_t(-2.376146124821733192409666, -2.63956852027813624177239618242498),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::beta(i, h), z_t(1.0794242492709257801356755, -1.41003240566416083828875193632476),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::beta(i, T(0.5)), z_t(1.0794242492709257801356755, -1.41003240566416083828875193632476),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::beta(i, o), -i, tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::beta(o, T(0.5)), z_t(2.0), tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::beta(i, 2 * i), z_t(-2.891319789417896828442673, -1.04053537453220827379062834820451),
                     tts::prec<T>());
};
