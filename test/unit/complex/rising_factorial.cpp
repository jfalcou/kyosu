...
<typename T>(T const& a0, T const& a1, T const& a2, T const& a3)
{
  ...
  auto rf = [](auto z0, auto z1) {
    auto zz0 = kyosu::complex(z0);
    return kyosu::if_else(kyosu::is_eqz(z1), kyosu::one, kyosu::tgamma(zz0 + z1) * kyosu::tgamma_inv(zz0));
  };
  ...
  z_t i = tcx(0, 1);
  z_t o = tcx(1, 0);
  TTS_RELATIVE_EQUAL(kyosu::rising_factorial(i, i), tcx(0.133408195318916183621, +0.06139167700318597540461),
                     tts::prec<T>());
  TTS_RELATIVE_EQUAL(kyosu::rising_factorial(i, T(0.5)), tcx(0.60672961239287904055952, +0.7925599369551407652246),
                     tts::prec<T>());
  ...
  TTS_RELATIVE_EQUAL(kyosu::rising_factorial(i, 2 * i),
                     tcx(0.00533756125402432030171473451274, +0.024348078097112309674412896870731), tts::prec<T>());
};
