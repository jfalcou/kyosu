...
<typename T>(T const& a0, T const& a1)
{
  ...
  for (size_t i = 0; i < a0.size(); ++i)
  {
    auto e = a0[i];
    auto f = a1[i];
    TTS_RELATIVE_EQUAL(kyosu::cot(kc_t(e, f)), kyosu::rec(cv(std::tan(c_t(e, f)))), tts::prec<T>());
  }
};

TTS_CASE_WITH("Check behavior of cot on wide", kyosu::simd_real_types, tts::randoms(1, 10), tts::randoms(-3, 3))
<typename T>(T const& a0, T const& a1)
{
  using e_t = T;
  ...
  TTS_RELATIVE_EQUAL(kyosu::cot(ke_t{a0, a1}), e, tts::prec<T>());
};
