...
<typename T>(T const& a0, T const& a1)
{
  ...
  for (size_t i = 0; i < a0.size(); ++i)
  {
    ...
    TTS_RELATIVE_EQUAL(kyosu::cosh(kc_t(e, f)), cv(std::cosh(c_t(e, f))), tts::prec<T>());
  }
};

TTS_CASE_WITH("Check behavior of cosh on wide", kyosu::simd_real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
<typename T>(T const& a0, T const& a1)
{
  using e_t = T;
  ...
  TTS_RELATIVE_EQUAL(kyosu::cosh(ke_t{a0, a1}), e, tts::prec<T>());
};
