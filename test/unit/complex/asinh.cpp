...
<typename T>(T const& a0, T const& a1)
{
  ...
  for (size_t i = 0; i < a0.size(); ++i)
  {
    auto e = a0[i];
    auto f = a1[i];
    TTS_RELATIVE_EQUAL(kyosu::asinh(kc_t(e, f)), cv(std::asinh(c_t(e, f))), tts::prec<e_t>())
      << " <- " << kc_t(e, f) << '\n';
  }
};

TTS_CASE_WITH("Check behavior of asinh on wide", kyosu::simd_real_types, tts::randoms(-10, 10), tts::randoms(-10, 10))
<typename T>(T const& a0, T const& a1)
{
  using e_t = T;
