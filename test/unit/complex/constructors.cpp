...
<typename T>(tts::type<T>)
{
  ...
  eve::wide<kyosu::as_complex_t<T>> z_l(fill);
  eve::wide<kyosu::as_complex_t<T>> z_ll(fill_r, fill_i);
  eve::wide<kyosu::as_complex_t<T>> z_lc(fill_r, T{1.25});
  eve::wide<kyosu::as_complex_t<T>> z_cl(T{8.8},fill_i);
...
<typename T>(tts::type<T>)
{
  ...
  kyosu::as_complex_t<T> z_s{spi,so};
  auto              z_sc{z_s};
  kyosu::as_complex_t<T> z_sa;
  z_sa = z_s;

  eve::wide<kyosu::as_complex_t<T>>  z_v{wo,wpi};
  auto                          z_vc{z_v};
  eve::wide<kyosu::as_complex_t<T>>  z_va;
  z_va = z_v;
