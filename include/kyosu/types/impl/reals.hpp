...
namespace kyosu::_
{
  ...
  template<eve::floating_value T>
  constexpr auto to_quaternion_(EVE_EXPECTS(eve::cpu_), T r)         noexcept  { return as_cayley_dickson_n_t<4,T>(r, 0, 0, 0); }

  template<eve::floating_value T0, eve::floating_value T1>
  constexpr auto to_quaternion_(EVE_EXPECTS(eve::cpu_), T0 r, T1 i)  noexcept -> as_cayley_dickson_n_t<4, decltype(eve::add(r, i))>
  {
  ...
  constexpr auto to_quaternion_(EVE_EXPECTS(eve::cpu_), T0 r, T1 i, T2 j, T3 k)  noexcept -> as_cayley_dickson_n_t<4, decltype(eve::add(r, i, j, k))>
  {
