...
namespace kyosu::_
{
  ...
  template<concepts::cayley_dickson T> struct sema<T>
  {
    using type = eve::wide<eve::underlying_type_t<T>>;
  };
...
{
  //====================================================================================================================
  //! @defgroup kyosu_traits Cayley-Dickson traits
  //! @brief Type traits reporting on the algebra a type belongs to.
  //====================================================================================================================

  //====================================================================================================================
  //! @addtogroup kyosu_traits
  //! @{
  //====================================================================================================================
  ...
  template<typename T, unsigned int Dim> struct as_real_type<cayley_dickson<T, Dim>>
  {
    using type = T;
  };
  ...
  template<unsigned int Dim, typename... Ts> struct as_cayley_dickson_n;
  ...
  template<typename... Ts> struct as_cayley_dickson_n<1, Ts...> : eve::common_value<_::sema_t<Ts>...>
  {
  };
#endif
...
namespace kyosu
{
  ...
  //! @tparam Ts  Types used to compute the Cayley-Dickson underlying type.
  //====================================================================================================================
  template<typename... Ts> using as_cayley_dickson_t = typename as_cayley_dickson<Ts...>::type;

  template<auto Callable, typename... Ts>
  using expected_result_t =
    as_cayley_dickson_n_t<std::max({dimension_v<Ts>...}), decltype(Callable(std::declval<as_real_type_t<Ts>>()...))>;
  ...
  {
    using base = std::conditional_t<!O::contains(kyosu::real_only), complexify<T>, eve::_::always<T>>;
