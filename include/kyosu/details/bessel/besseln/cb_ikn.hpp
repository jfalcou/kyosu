...
namespace kyosu::_
{
  ...
  //  cb_in just last
  //===-------------------------------------------------------------------------------------------
  template<eve::integral_scalar_value N, typename Z> KYOSU_FORCEINLINE auto cb_in(N n, Z z) noexcept
  {
  ...
  //  cb_kn just last
  //===-------------------------------------------------------------------------------------------
  template<eve::integral_scalar_value N, typename Z> KYOSU_FORCEINLINE auto cb_kn(N n, Z z) noexcept
  {
    ...
    auto r = if_else(argzlt0, cpi * cb_hn<1>(n, z * epio2), cmi * cb_hn<2>(n, z * empio2));
  ...
  //===-------------------------------------------------------------------------------------------
  template<eve::integral_scalar_value N, typename Z, std::size_t S> Z cb_in(N n, Z z, std::span<Z, S> ris)
  {
    using e_t = as_real_type_t<Z>;
    auto miton = [](N n) {
      if (n % 4 == 0) return complex(eve::one(eve::as<e_t>()));
    ...
    for (int i = 1; i < int(nn); ++i) ris[i] *= miton(i);
    return jn * miton(n);
  }
}
