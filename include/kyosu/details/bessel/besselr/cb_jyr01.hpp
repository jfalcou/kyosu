...
namespace kyosu::_
{
  ...
  template<eve::floating_scalar_value N, typename Z> auto cb_jyr01(N v, Z z) noexcept
  {
    ...
    using u_t = eve::underlying_type_t<Z>;
    auto const eps = 16 * eve::eps(as<u_t>());
    auto const hlf = eve::half(as<u_t>());
    auto const two = u_t(2);
    auto const three = u_t(3);
    auto const four = u_t(4);
    auto const quarter = u_t(0.25);
    auto const twoopi = eve::two_o_pi(as<u_t>());
    auto const invpi = eve::inv_pi(as<u_t>());
    ...
    Z const cone = kyosu::one(as<Z>());
    Z const cnan = kyosu::nan(as<Z>());
    ...
    auto br_gt12 = [&]() {
      ...
      auto cjyv01 = [&](auto v0p) {
        ...
        {
          crp =
            u_t(-0.78125e-2) * crp * (vv - sqr(four * k - three)) * (vv - sqr(dec(four * k))) / (k * dec(two * k) * z2);
        ...
        {
          crq =
            u_t(-0.78125e-2) * crq * (vv - sqr(dec(four * k))) * (vv - sqr(inc(four * k))) / (k * inc(two * k) * z2);
          cqz += crq;
