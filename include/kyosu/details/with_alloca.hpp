...
namespace kyosu::_
{
  template<typename T, std::invocable<std::span<T>> F> decltype(auto) with_alloca(auto size, F f)
  {
  ...
  template<typename T, std::invocable<std::span<T>, std::span<T>> F> decltype(auto) with_alloca(auto size, F f)
  {
