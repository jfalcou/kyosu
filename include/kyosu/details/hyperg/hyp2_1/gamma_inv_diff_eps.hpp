...
namespace kyosu::_
{
  auto gamma_inv_diff_eps(auto z, auto eps, auto notdone)
  {
    ...
    auto br_epspzneqz = [&]() { // (epspz != z)
      auto br_is_z_negative_integer = [&]() { return -tgamma_inv(epspz) / eps; };
      auto br_is_epspz_negative_integer = [&]() { return tgamma_inv(z) / eps; };
      ...
      auto br_else = [&]() { return r_t(gamma_ratio_diff_small_eps(epspz, -eps, notdone) * tgamma_inv(z)); };
    ...
    auto br_final = [&]() { return gamma_ratio_diff_small_eps(z, eps, notdone) * kyosu::tgamma_inv(epspz); };
