  ...
  auto hyp_ps_one(auto a, auto b, auto c, auto one_minus_z)
  {
    ...
    if (eve::any(invalid_A_first_term))
    {
      A_term = kyosu::if_else(invalid_A_first_term,
                              kyosu::exp(kyosu::log_gamma(c) - kyosu::log_gamma(eps_pa_pm) -
                                         kyosu::log_gamma(eps_pb_pm) + log_A_sum_init(m, eps)),
                              A_term);
    ...
    r_t B_first_term(B_sum_init_ps_one(a, b, c, gamma_c, gamma_inv_one_meps, gamma_inv_eps_pa_pm, gamma_inv_eps_pb_pm,
    ...
    for (int n = 0; n < maxit; ++n)
    {
      ...
      B_term = one_minus_z *
               (B_term * prod1 / prod2 +
                B_extra_term * (prod3 / n_pm_p1 - a_pm_pn - b_pm_pn - eps + prod1 / n_p1) / (eps_pm_p1_pn * n_p1_meps));
      B_sum += B_term;
      ...
      possible_false_cv = kyosu::if_else(possible_false_cv && (n >= min_n),
                                         eve::is_gtz(dcv_calc(cv_poly1_der_tab, u_t(n))) ||
                                           eve::is_gtz(dcv_calc(cv_poly2_der_tab, u_t(n))),
                                         possible_false_cv);
    }
    auto pAB_sum = (A_sum + B_sum) * phase;
    auto r = kyosu::if_else(kyosu::is_eqz(eps), pAB_sum, pAB_sum * pi_eps / kyosu::sinpi(eps));
    return r;
  }
}
