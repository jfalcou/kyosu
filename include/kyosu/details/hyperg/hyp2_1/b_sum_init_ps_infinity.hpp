...
namespace kyosu::_
{
  ...
  auto B_sum_init_ps_infinity(auto a,
                              auto c,
                              auto gamma_c,
                              auto gamma_inv_cma,
                              ...
                              auto notdone) noexcept
  {
    ...
    auto br_eps_le = [&]() { //if (inf_norm_eps <=  0.1)
      ...
      auto gamma_inv_eps_pm_p1 =
        if_else(is_eps_non_zero, (phase * kyosu::sinpi(eps) / (pi_eps_pm * gamma_inv_one_meps_mm)), gamma_inv_mp1);
      ...
      auto prod1 = gamma_prod1 * gamma_inv_mp1 *
                   (gamma_inv_diff_eps(kyosu::one(as<r_t>()), eps, notdone) * prod_eps_pa_mc_p1 -
