...
namespace kyosu::_
{
  auto B_sum_init_ps_one(auto a,
                         auto b,
                         auto c,
                         auto gamma_c,
                         auto gamma_inv_one_meps,
                         auto gamma_inv_eps_pa_pm,
                         auto gamma_inv_eps_pb_pm,
                         auto one_minus_z,
                         auto m,
                         auto eps,
                         auto notdone) noexcept
  {
    using r_t = decltype(a + b + c + gamma_c + gamma_inv_one_meps + gamma_inv_eps_pa_pm + gamma_inv_eps_pb_pm +
                         one_minus_z + eps + m);
    ...
    auto br_le = [&]() {
      ...
      auto gamma_inv_eps_pm_p1 =
        if_else(is_eps_non_zero, (phase * sinpi(eps) / (pi_eps_pm * gamma_inv_one_meps_mm)), (gamma_inv_mp1));
      ...
      auto prod1 = gamma_inv_eps_pa_pm * gamma_inv_eps_pb_pm *
                   (gamma_inv_mp1 * gamma_inv_diff_eps(eve::one(eve::as<u_t>()), -eps, notdone) +
                    gamma_inv_diff_eps(inc(m), eps, notdone));
      auto prod2 = gamma_inv_eps_pm_p1 * (gamma_inv_eps_pb_pm * gamma_inv_diff_eps(a_pm, eps, notdone) +
                                          gamma_inv_a_pm * gamma_inv_diff_eps(b_pm, eps, notdone));
                         ...
                         auto eps) noexcept
  {
    using r_t = decltype(a + b + c + gamma_c + gamma_inv_one_meps + gamma_inv_eps_pa_pm + gamma_inv_eps_pb_pm +
                         one_minus_z + eps + m);
    return B_sum_init_ps_one(a, b, c, gamma_c, gamma_inv_one_meps, gamma_inv_eps_pa_pm, gamma_inv_eps_pb_pm,
