...
namespace kyosu::_
{
  auto hyp_ps_infinity(auto a, auto b, auto c, auto z, auto notdone) noexcept
  {
    ...
    {
      A_term = if_else(
        invalid_A_first_term,
        kyosu::exp(kyosu::log_gamma(c) - kyosu::log_gamma(cma) - kyosu::log_gamma(b) + log_A_sum_init(m, eps)), A_term);
...
//      auto eps_pm_pn = eps_pm + n; //TO KILL
      ...
      B_term = z_inv * (B_term * prod1 / prod2 + B_extra_term *
    ...
    return if_else(kyosu::is_eqz(eps), (phase * pow_mz_ma * (A_sum + B_sum)),
