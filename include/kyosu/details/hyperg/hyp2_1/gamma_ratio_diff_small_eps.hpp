...
namespace kyosu::_
{
  ...
  auto gamma_ratio_diff_small_eps(auto z, auto eps, auto notdone) -> decltype(z + eps)
  {
    ...
    constexpr u_t c[15] = {0.99999999999999709182,     57.156235665862923517,      -59.597960355475491248,
    ...
    auto br_0 = [&](auto) {
      ...
      auto r = kyosu::expm1(z_m_0p5 * kyosu::log1p(eps / z_pg_m0p5) + eps * kyosu::log(eps_pz_pg_m0p5) - eps +
                            kyosu::log1p(-eps * sum_num / sum_den)) /
               eps;
      ...
      else
      {
        if constexpr (kyosu::concepts::real<r_t>)
          return if_else(
            zero_eps, (z_m_0p5 / z_pg_m0p5 + eve::log(eps_pz_pg_m0p5) - eve::one(eve::as<u_t>()) - sum_num / sum_den),
            r);
        else
          return if_else(
            zero_eps, (z_m_0p5 / z_pg_m0p5 + kyosu::log(eps_pz_pg_m0p5) - eve::one(eve::as<u_t>()) - sum_num / sum_den),
            r);
      }
    };
    ...
    auto br_1 = [&](auto test, auto) {
      ...
      else
      {
        return if_else(zero_eps && notdone,
                       gamma_ratio_diff_small_eps(kyosu::oneminus(z), zero(as<r_t>()), notdone && !test) -
                         eve::pi(eve::as<u_t>()) * kyosu::cotpi(z - n),
                       r);
      }
    };
