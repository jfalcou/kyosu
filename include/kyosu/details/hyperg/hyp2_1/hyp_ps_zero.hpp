...
namespace kyosu::_
{
  auto hyp_ps_zero(auto a, auto b, auto c, auto z, auto notdone)
  {
    ...
    if constexpr (eve::scalar_value<r_t>)
    {
      r_t term(1);
      r_t sum(1);
      int const na = eve::abs(eve::nearest(real(a)));
      int const nb = eve::abs(eve::nearest(real(b)));
      ...
      else
      {
        ...
        int const min_n = min_n_calc(dcv_tab);
    ...
    else
    {
      ...
      auto br_else = [&]() {
        ...
        for (int n = 0; n < maxit; ++n)
        {
          ...
          possible_false_cv = kyosu::if_else(possible_false_cv && (n >= min_n), eve::is_gtz(dcv_calc(dcv_tab, u_t(n))),
                                             possible_false_cv);
        }
      ...
      if (eve::any(notdone))
      {
        ...
        if (eve::any(notdone))
        {
          ...
          if (eve::any(notdone))
          {
            ...
            if (eve::any(notdone)) { last_interval(br_else, notdone, r); }
          }
        }
      }
      return r;
