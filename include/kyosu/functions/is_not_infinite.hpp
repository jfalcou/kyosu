...
namespace kyosu
{
  template<typename Options> struct is_not_infinite_t : eve::elementwise_callable<is_not_infinite_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::is_not_infinite(z);
    else return kumi::all_of(z, [](auto const& e) { return eve::is_not_infinite(e); });
  }
}
