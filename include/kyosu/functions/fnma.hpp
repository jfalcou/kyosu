...
namespace kyosu
{
  template<typename Options> struct fnma_t : eve::strict_elementwise_callable<fnma_t, Options, raw_option, pedantic_option>
  {
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto fnma_(KYOSU_DELAY(), O const&, Z0 z0, Z1 z1, Z2 z2) noexcept
  {
    if constexpr (concepts::real<Z0> && concepts::real<Z1> && concepts::real<Z2>) return eve::fnma(z0, z1, z2);
    else return z2 - z0 * z1;
  }
}
