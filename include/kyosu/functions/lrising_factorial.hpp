...
namespace kyosu
{
  ...
  struct lrising_factorial_t : eve::strict_elementwise_callable<lrising_factorial_t, Options, raw_option, pedantic_option, real_only_option>
  {
  ...
  //!       in which case the parameters must be floating values,  implying a Nan result if the theoretical result is
  //!       not real.
  //!
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto lrising_factorial_(KYOSU_DELAY(), O const& o, Z0 z0, Z1 z1) noexcept
  {
    ...
    else return kyosu::log[o](kyosu::rising_factorial[o](z0, z1));
  }
  ...
  KYOSU_FORCEINLINE constexpr auto lrising_factorial_(
    KYOSU_DELAY(), C const& cx, O const& o, Z0 z0, Z1 z1, K... k) noexcept
  requires(!O::contains(real_only))
  {
    return eve::_::mask_op(cx, eve::_::return_2nd, complex(z0), lrising_factorial(z0, z1, k...));
  }
}
