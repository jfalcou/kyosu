...
namespace kyosu
{
  ...
  struct kolmmean_t : eve::strict_elementwise_callable<kolmmean_t, Options, eve::pedantic_option, eve::kahan_option>
  {
    ...
    operator()(F f,  G g, Tup const& t) const noexcept requires(kumi::size_v<Tup> >= 1)
...
namespace kyosu::_
{
  ...
  template<typename F, typename G, concepts::cayley_dickson_like T0, concepts::cayley_dickson_like ... Ts, eve::callable_options O>
  EVE_FORCEINLINE constexpr auto
  kolmmean_(KYOSU_DELAY(), O const & o, F f, G g, T0 t0, Ts... args) noexcept
  {
    if constexpr(sizeof...(Ts) == 0)
      return t0;
    else
      return g(average[o](f(t0), f(args)...));
  }

  template<typename F, typename G, kumi::non_empty_product_type Tup, eve::callable_options O>
  EVE_FORCEINLINE constexpr auto
  kolmmean_(KYOSU_DELAY(), O const & o, F f, G g, Tup tup) noexcept
