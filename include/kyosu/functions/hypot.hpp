...
namespace kyosu
{
  template<typename Options> struct hypot_t : eve::strict_elementwise_callable<hypot_t, Options, eve::pedantic_option>
  {
    ...
    KYOSU_FORCEINLINE constexpr auto operator()(Z0 z0, Zs... zs) const noexcept
      -> decltype(eve::hypot(real(z0), real(zs)...))
    {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z0> && (... && concepts::real<Zs>)) return eve::hypot[o](z0, zs...);
    else return eve::hypot[o](kumi::flatten(kumi::make_tuple(z0, zs...)));
  }
}
