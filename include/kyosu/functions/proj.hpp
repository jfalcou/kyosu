...
namespace kyosu
{
  template<typename Options> struct proj_t : eve::elementwise_callable<proj_t, Options, raw_option, pedantic_option>
  {
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto proj_(KYOSU_DELAY(), O const&, C c) noexcept
  {
    ...
    if constexpr (kyosu::concepts::real<C>) return if_else(eve::is_infinite(c), eve::inf(as(c)), c);
    else if constexpr (kyosu::concepts::complex<C>)
      return if_else(is_infinite(c), complex(eve::inf(real_t{}), eve::copysign(eve::zero(real_t{}), imag(c))), c);
    else
    {
      auto isinf = is_infinite(c);
      auto tmp = eve::if_else(isinf, eve::inf(real_t{}), real(c));
      auto setpure = [isinf](auto& x) { x = eve::if_else(isinf, eve::copysign(eve::zero(real_t{}), x), x); };
