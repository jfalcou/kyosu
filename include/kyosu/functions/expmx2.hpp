...
namespace kyosu
{
  ...
  struct expmx2_t : eve::elementwise_callable<expmx2_t, Options, raw_option, pedantic_option, real_only_option>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (O::contains(radpi)) return kyosu::exp[o.drop(radpi)](-sqr(eve::pi(eve::as_element<Z>()) * z));
    else return kyosu::exp[o](-sqr(z));
  }
}
