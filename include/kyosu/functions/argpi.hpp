...
namespace kyosu
{
  template<typename Options> struct argpi_t : eve::elementwise_callable<argpi_t, Options>
  {
  ...
  //!   **Return value**
  //!
  //!     - For floating  value input, returns \f$0\f$ or \f$1\f$ according to the non negativity of `z` (argpi of minus
  //!     zero is 1).
  //!     - For complex  value input, returns elementwise the argument of the complex number in \f$\pi\f$ multiples i.e.
  //!     `eve::atan2pi[pedantic](imag(z),real(z))`.
  //!     - For general cayley_dickson,  returns \f$\mathrm{atan2pi}(\mathrm{sign}(z_1)|\underline{z}|, z_0)\f$ where
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto argpi_(KYOSU_DELAY(), O const&, Z v) noexcept
  {
    ...
    {
      return eve::if_else(eve::is_nan(v), eve::allbits,
                          eve::if_else(eve::is_positive(v), eve::zero, eve::one(eve::as(v))));
    }
