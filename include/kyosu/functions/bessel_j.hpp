...
namespace kyosu
{
  ...
  template<typename Options> struct bessel_j_t : eve::strict_elementwise_callable<bessel_j_t, Options, raw_option, pedantic_option, eve::spherical_option, eve::cylindrical_option>
  {
  ...
  //!      template<eve;scalar_value N, complex_like T, size_t S> constexpr auto bessel_j(N n, T z, std::span<Z, S> cis)            noexcept; //2
  //!      template<eve;scalar_value N, cayley_dickson_like T>    constexpr auto bessel_j[spherical](N n, T z)                      noexcept; //3
  //!      template<eve;scalar_value N, complex_like T, size_t S> constexpr auto bessel_j[spherical](N n, T z, std::span<Z, S> sis) noexcept; //4
  //!   }
  //!   @endcode
  //!
  //!   **Parameters**
  ...
  //!   *  [Wolfram MathWorld: Bessel Function of the First
  //!   Kind](https://mathworld.wolfram.com/BesselFunctionoftheFirstKind.html)
  //!   *  [Wolfram MathWorld: Spherical Bessel Function of the First
  //!   Kind](https://mathworld.wolfram.com/SphericalBesselFunctionoftheFirstKind.html)
...
namespace kyosu::_
{
  template<typename N, typename Z, eve::callable_options O> KYOSU_FORCEINLINE constexpr auto bessel_j_(KYOSU_DELAY(), O const& o, N n, Z z) noexcept
  {
    if constexpr (concepts::complex<Z>)
    {
      if constexpr (eve::integral_scalar_value<N>)
      {
        if constexpr (O::contains(eve::spherical))
        {
          ...
          else return _::sb_jn(n, z);
        }
        else return _::cb_jn(n, z);
      }
      else if constexpr (eve::floating_scalar_value<N>)
      {
        if constexpr (O::contains(eve::spherical)) return _::sb_jr(n, z);
        else return _::cb_jr(n, z);
      }
    }
    else return _::cayley_extend_rev(kyosu::bessel_j, n, z);
  }
  ...
  KYOSU_FORCEINLINE constexpr auto bessel_j_(KYOSU_DELAY(), O const& o, N n, Z z, JS& js) noexcept
  {
    if constexpr (eve::integral_scalar_value<N>)
    {
      if constexpr (O::contains(eve::spherical))
      {
        ...
        else return _::sb_jn(n, z, js);
      }
      else return _::cb_jn(n, z, js);
    }
    else
    {
      if constexpr (O::contains(eve::spherical)) return _::sb_jr(n, z, js);
      else return _::cb_jr(n, z, js);
    }
  }
}
