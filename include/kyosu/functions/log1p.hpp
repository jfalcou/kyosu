...
namespace kyosu
{
  ...
  struct log1p_t : eve::strict_elementwise_callable<log1p_t, Options, raw_option, pedantic_option, real_only_option>
  {
  ...
  //!       eve::log1p implying a Nan result if the theoretical result is not real.
...
namespace kyosu::_
{
  template<typename Z, eve::callable_options O> constexpr auto log1p_(KYOSU_DELAY(), O const& o, Z z) noexcept
  {
    if constexpr (O::contains(real_only) && concepts::real<Z>) return eve::log1p[o.drop(real_only)](z);
    else if constexpr (concepts::real<Z>) return kyosu::log1p[o](complex(z));
  ...
  {
    if constexpr (kyosu::concepts::real<Z>) return log1p[o](complex(z));
    else if constexpr (kyosu::concepts::complex<Z>)
    {
  ...
  KYOSU_FORCEINLINE constexpr auto log1p_(KYOSU_DELAY(), C const& cx, O const&, Z z, K... k) noexcept
  requires(!O::contains(real_only))
  {
    return eve::_::mask_op(cx, eve::_::return_2nd, complex(z), log1p(z, k...));
  }
