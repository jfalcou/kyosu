...
namespace kyosu
{
  template<typename Options> struct if_else_t : eve::callable<if_else_t, Options>
  {
    ...
    KYOSU_FORCEINLINE constexpr expected_result_t<eve::if_else, M, T, F> operator()(M const& m,
                                                                                    T const& t,
    ...
    template<typename M, typename T, typename F>
    KYOSU_FORCEINLINE constexpr auto operator()(M const& m, T const& t, F const& f) const noexcept
  ...
  KYOSU_FORCEINLINE constexpr auto if_else_(KYOSU_DELAY(), O const& o, M const& m, Z1 const& t, Z2 const& f) noexcept
  {
    if constexpr (concepts::cayley_dickson<Z1> && concepts::cayley_dickson<Z2>)
    {
      ...
      return ret_t{kumi::map([&](auto const& v, auto const& w) { return eve::if_else(m, v, w); },
                             kyosu::convert(t, eve::as_element<type>{}), kyosu::convert(f, eve::as_element<type>{}))};
    }
    else
    {
      auto parts = [&]() {
