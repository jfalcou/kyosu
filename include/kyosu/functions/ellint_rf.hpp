...
namespace kyosu
{
  ...
  template<typename Options> struct ellint_rf_t : eve::elementwise_callable<ellint_rf_t, Options, raw_option, pedantic_option, eve::threshold_option>
  {
    template<eve::floating_value T0, eve::floating_value T1, eve::floating_value T2>
    constexpr KYOSU_FORCEINLINE as_cayley_dickson_like_t<T0, T1> operator()(T0 a, T1 b, T2 c) const noexcept
    {
...
namespace kyosu::_
{
  ...
  constexpr auto ellint_rf_(KYOSU_DELAY(), O const& o, T x, T y, T z) noexcept
  {
    ...
    return (kyosu::fma(e3, kyosu::fma(e3, c1, c0),
