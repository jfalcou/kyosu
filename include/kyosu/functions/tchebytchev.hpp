...
namespace kyosu
{
  ...
  struct tchebytchev_t : eve::strict_elementwise_callable<tchebytchev_t, Options
                                                          , eve::successor_option, kind_1_option, kind_2_option>
  {
    template<concepts::real N, concepts::cayley_dickson Z>
    KYOSU_FORCEINLINE constexpr auto  operator()(N n, Z z) const noexcept -> decltype(n+z)
    {
      using r_t = decltype(n+z);
      using u_t = eve::underlying_type_t<r_t>;
      if constexpr(!Options::contains(kind_2))
      {
        auto r = kyosu::hypergeometric(oneminus(z)*eve::half(as(n)), kumi::tuple{-n, n}, kumi::tuple{u_t(0.5)});
        //        auto r = kyosu::cos(n*kyosu::acos(z));
        return if_else(is_real(z), complex(real(r)), r);
      }
      else
      {
        auto r = kyosu::inc(n)*hypergeometric(oneminus(z)*eve::half(as(n)), kumi::tuple{-n, n+2}, kumi::tuple{u_t(1.5)});
        return if_else(is_real(z), complex(real(r)), r);
//         auto sqtz = kyosu::sqrt(kyosu::dec(kyosu::sqr(z)));
//         return kyosu::average(pow(sqtz+z, n+1), -pow(z-sqtz, n+1))/sqtz;
      }
    }
...
//!                                                           kyosu::concepts::real auto tn,
//!                                                           kyosu::concepts::real auto tnm1)            noexcept; // 4
//!   }
//!   @endcode
//!
//!   **Parameters**
