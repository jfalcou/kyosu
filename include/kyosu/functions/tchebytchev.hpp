...
namespace kyosu
{
  ...
  struct tchebytchev_t : eve::strict_elementwise_callable<tchebytchev_t,
                                                          Options,
                                                          ...
                                                          eve::kind_2_option>
  {
    ...
    KYOSU_FORCEINLINE constexpr auto operator()(N nn, Z zz) const noexcept -> decltype(nn + zz)
    {
      ...
      {
        auto r =
          kyosu::hypergeometric(oneminus(z) * eve::half(eve::as<u_t>()), kumi::tuple{-n, n}, kumi::tuple{u_t(0.5)});
        return if_else(is_real(z), complex(real(r)), r);
      }
      else
      {
        auto r = kyosu::inc(n) *
                 hypergeometric(oneminus(z) * eve::half(eve::as<u_t>()), kumi::tuple{-n, n + 2}, kumi::tuple{u_t(1.5)});
        return if_else(is_real(z), complex(real(r)), r);
      }
    }
