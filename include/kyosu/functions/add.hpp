...
namespace kyosu
{
  ...
  struct add_t : kyosu::strict_tuple_callable<add_t, Options, raw_option, pedantic_option, eve::kahan_option>
  {
  ...
  //!
  //! @note If all elements are real typed the result will be real typed, using a call to `eve::add`
  //!
  //!  @groupheader{Example}
  //!  @godbolt{doc/add.cpp}
  //======================================================================================================================
...
namespace kyosu::_
{
  ...
  EVE_FORCEINLINE constexpr auto add_(KYOSU_DELAY(), O const& o, T0 const& t0, Ts const&... ts) noexcept
  {
    ...
    if constexpr (concepts::real<r_t>) return eve::add[o](t0, ts...);
    else if constexpr (sizeof...(Ts) == 0) return t0;
    else return kumi::map(eve::add[o], r_t(t0), r_t(ts)...);
  }
