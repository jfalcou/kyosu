...
namespace kyosu
{
  ...
  struct ellint_fe_t : eve::strict_elementwise_callable<ellint_fe_t,
                                                        Options,
                                                        eve::modular_option,
                                                        ...
                                                        eve::threshold_option>
  {
    ...
    constexpr KYOSU_FORCEINLINE auto operator()(T0 a, T1 b) const noexcept -> decltype(eve::zip(T0(), T0()))
    {
...
namespace kyosu::_
{
  ...
  constexpr auto ellint_fe_(KYOSU_DELAY(), O const& o, Z u, M m) noexcept -> decltype(eve::zip(Z(), Z()))
  {
    ...
    else
    {
      ...
      if (eve::all(is_real(u)))
        return eve::zip(kyosu::complex(eve::ellint_1[o](phi, m)), kyosu::complex(eve::ellint_2[o](phi, m)));
      auto m2 = eve::sqr(m);
      auto thresh = eve::eps(eve::as(phi));
      phi = if_else(eve::abs(phi) < thresh, thresh, phi); //avoiding singularity at 0

      auto b =
        -(eve::sqr(eve::cot(phi)) + m2 * eve::sqr(eve::sinh(psi) * eve::csc(phi)) - 1 + m2); //*eve::half(eve::as(phi));
      ...
      lambda = eve::sign_alternate(eve::floor(2 * phi * eve::inv_pi(eve::as(phi)))) * lambda +
               eve::pi(eve::as(phi)) *
                 eve::ceil(phi / eve::pi(eve::as(phi)) - eve::half(eve::as(phi)) + eve::eps(eve::as(phi)));
      mu = eve::sign(psi) * mu;
