...
namespace kyosu
{
  template<typename Options> struct pow_t : eve::callable<pow_t, Options, real_only_option>
  {
    ...
    KYOSU_FORCEINLINE constexpr auto operator()(Z0 z0, Z1 z1) const noexcept
      -> complexify_t<kyosu::as_cayley_dickson_like_t<Z0, Z1>>
    ...
    KYOSU_FORCEINLINE constexpr auto operator()(Z0 z0, Z1 z1) const noexcept -> complexify_t<Z0>
    requires(!Options::contains(real_only))
    {
      if constexpr (concepts::real<Z0>) return (*this)(complex(z0), z1);
  ...
  //!         in which case the parameters must be  floating values,  the real part of the result will the same as an
  ...
  //!         If the first parameter is integral typed the result is real typed.
  //!
...
namespace kyosu::_
{
  ...
  requires(eve::integral_scalar_value<C1>)
  {
    ...
    else
    {
      ...
      {
        if constexpr (kyosu::concepts::real<C0>) return r_t(eve::pow(c0, c1));
        ...
        else
        {
          ...
          else
          {
            ...
            return kyosu::if_else(kyosu::is_eqz(cc0),
                                  eve::if_else(kyosu::is_eqz(cc1), eve::one(eve::as<u_t>()), eve::zero(eve::as<u_t>())),
                                  r);
          }
        }
      }
