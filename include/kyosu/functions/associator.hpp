...
namespace kyosu
{
  ...
  struct associator_t : eve::strict_elementwise_callable<associator_t, Options, eve::pedantic_option>
  {
    ...
    KYOSU_FORCEINLINE constexpr as_cayley_dickson_like_t<Z0, Z1, Z2> operator()(Z0 const& c0,
                                                                                Z1 const& c1,
                                                                                Z2 const& c2) const noexcept
    {
  ...
  //!     2.  Same,  but always does the computation. (up to quaternions, non zero result is merely the
...
namespace kyosu::_
{
  ...
  {
    if constexpr (O::contains(eve::pedantic)) return (c0 * c1) * c2 - c0 * (c1 * c2);
    else
    {
