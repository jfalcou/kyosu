...
namespace kyosu
{
  template<typename Options> struct acos_t : eve::strict_elementwise_callable<acos_t, Options, real_only_option>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (O::contains(real_only) && concepts::real<Z>) return eve::acos(a0);
    else if constexpr (concepts::real<Z>) return kyosu::acos(complex(a0));
