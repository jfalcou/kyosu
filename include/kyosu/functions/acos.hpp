...
namespace kyosu
{
  ...
  struct acos_t : eve::strict_elementwise_callable<acos_t, Options, raw_option, pedantic_option, real_only_option, radpi_option>
  {
...
namespace kyosu
{
  namespace _
  {
    ...
    EVE_NOINLINE constexpr auto acos_(KYOSU_DELAY(), O const& o, Z a0) noexcept
    {
      ...
      else if constexpr (concepts::real<Z>) return kyosu::acos[o](complex(a0));
      else if constexpr (concepts::complex<Z>)
      {
        ...
        rtype apx = a + x;
        r = eve::if_else(lexone, eve::atan(eve::sqrt(eve::half(eve::as(x)) * apx * (yy / (tr + xp1) + (ts - xm1))) / x),
        ...
        if (eve::any(not_in_safe_zone))
        {
          ...
          if (eve::any(eve::logical_and(zone1, not_in_safe_zone)))
          {
            ...
            rtype ii =
              eve::if_else(lexone, y / eve::sqrt(-xp1 * xm1),
                           eve::if_else((eve::valmax(eve::as(x)) / xp1 > xm1), eve::log1p(xm1 + eve::sqrt(xp1 * xm1)),
          ...
          if (eve::any(eve::logical_and(zone4, not_treated)))
          {
            ...
            i = eve::if_else(
              zone4, eve::log_2(eve::as(x)) + eve::log(y) + eve::half(eve::as(x)) * eve::log1p(eve::sqr(x / y)), i);
          }
    ...
    requires(!O::contains(real_only))
    {
      return eve::_::mask_op(cx, eve::_::return_2nd, complex(z), acos[o](z, k...));
    }

  }

  inline constexpr auto acospi = acos[radpi];
