...
namespace kyosu
{
  ...
  struct beta_t : eve::strict_elementwise_callable<beta_t, Options, raw_option, pedantic_option, real_only_option>
  {
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto beta_(KYOSU_DELAY(), O const&, Z0 z0, Z1 z1) noexcept
  {
    ...
    {
      if constexpr (O::contains(real_only)) return eve::beta(z0, z1);
      else return kyosu::beta(complex(z0), complex(z1));
    }
    else if constexpr (kyosu::concepts::complex_like<Z0> && kyosu::concepts::complex_like<Z1>)
      return tgamma(z0) * tgamma(z1) / tgamma(z0 + z1);
    else
      return kyosu::_::cayley_extend(tgamma, z0) * kyosu::_::cayley_extend(tgamma, z1) /
             kyosu::_::cayley_extend(tgamma, z0 + z1);
  }
