...
namespace kyosu
{
  template<typename Options> struct pow1p_t : eve::strict_elementwise_callable<pow1p_t, Options, raw_option, pedantic_option, real_only_option>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (O::contains(real_only)) return kyosu::inject(eve::pow1p(z0, z1));
    ...
    else return kyosu::pow(kyosu::inc(z0), z1);
  }
}
