...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto elliptic_fe_(KYOSU_DELAY(), O const& o, Z u, M m) noexcept
  {
    auto tol = [&](){
...
// phi(I) = eps;
// I = [];
    phi =  if_else(is_eqz(phi), eve::eps(as(phi)), phi);

// % finding the roots of the equation
// % X^2 - (cot(phi)^2+m*sinh(psi)^2*csc(phi)^2-1+m)X - (1-m)*cot(phi)^2 = 0
// b = -(cot(phi).^2 + m.*sinh(psi).^2.*csc(phi).^2-1+m);
// c = -(1-m).*cot(phi).^2;
    auto b = -(eve::sqr(eve::cot(phi)) + m*eve::sqr(eve::sinh(psi)*eve::csc(phi))-1+m)*eve::half(eve::as(b));
    auto c = -(1-m).*eve::cot(phi));
// X1 = -b/2 + sqrt(b.^2/4-c);
// I = find(X1>=0);
  auto X1 = -b+eve::sqrt(eve::sqr(b)-c);
...
// end
  ...
  auto mu     = eve::atan( eve::sqrt(rec(m)*eve::dec(eve::sqr(tan(phi)*eve::cot(lambda)))));
...
// mu     = sign(psi).*real(mu);
  auto lambda = eve::sign_alternate(eve::floor(2*phi*eve::inv_pi(eve::as(phi))))*lambda+eve::pi(eve::as(phi))*eve::ceil(phi/eve::pi(eve::as(phi))-eve::half(eve::as(phi))+eve::eps(eve::as(phi)))
    auto mu     = eve::sign(psi)*real(mu);
  // [F1(:),E1(:)] = elliptic12(lambda, m, tol);
// [F2(:),E2(:)] = elliptic12(mu, 1-m, tol);
    auto m2 = eve::sqr(m);
    auto mc = eve::sqrt(eve::oneminus(m2));
  auto f1 = eve::elliptic_1[eve::threshold = tol](phi, m);
  auto f2 = eve::elliptic_1[eve::threshold = tol](mu, mc);
  auto e1 = eve::elliptic_2[eve::threshold = tol](phi, m);
  auto e2 = eve::elliptic_2[eve::threshold = tol](mu, mc);
...
// sin_mu = sin(mu); cos_mu = cos(mu);
  auto [sin_lam, cos_lam] = sincos(lam);
  auto [sin_mu , cos_mu ] = sincos(mu);
...
// b3 = cos_mu.^2 + m.*sin_lam.^2.*sin_mu.^2;
  auto b1 = m2*sin_lam.*cos_lam*eve::sqr(sin_mu)*sqrt(eve::oneminus(m2*eve::sqr(sin_lam)));
  auto b2 = sin_mu*cos_mu*(1-m2*sin_lam.^2)*sqrt(1-oneminus(m2)*eve::sqr(s_mu));
  auto b3 = eve::sqr(cos_mu) + eve::sqr(m*sin_lam*sin_mu);
