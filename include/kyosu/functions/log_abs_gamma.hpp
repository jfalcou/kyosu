...
namespace kyosu
{
  template<typename Options> struct log_abs_gamma_t : eve::elementwise_callable<log_abs_gamma_t, Options, raw_option, pedantic_option>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::log_abs(eve::tgamma(z));
    else return kyosu::log_abs(kyosu::tgamma(z));
  }
}
