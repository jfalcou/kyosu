...
namespace kyosu
{
  template<typename Options> struct cotpi_t : eve::elementwise_callable<cotpi_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::cotpi(z);
    else if constexpr (concepts::complex<Z>)
    {
      auto r = kyosu::tanpi(z);
      r = kyosu::if_else(kyosu::is_infinite(r), eve::zero, kyosu::rec(r));
      r = kyosu::if_else(kyosu::is_real(z) && eve::is_flint(kyosu::real(z) * 2) && eve::is_not_flint(kyosu::real(z)),
                         eve::zero, r);
      return kyosu::if_else(kyosu::is_real(z), kyosu::complex(kyosu::real(r)), r);
    }
    else { return _::cayley_extend(kyosu::cotpi, z); }
  }
}
