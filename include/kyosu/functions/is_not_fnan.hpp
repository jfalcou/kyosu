...
namespace kyosu
{
  template<typename Options> struct is_not_fnan_t : eve::elementwise_callable<is_not_fnan_t, Options, raw_option, pedantic_option>
  {
  ...
  //!   namespace kyosu
  //!   {
  //!      template<kyosu::concepts::cayley_dickson_like T> constexpr auto is_not_fnan(T z) noexcept;
  //!   }
  //!   @endcode
  //!
  //!   **Parameters**
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto is_not_fnan_(KYOSU_DELAY(), O const&, Z z) noexcept
  {
    if constexpr (concepts::real<Z>) return eve::is_not_nan(z);
    else return kumi::any_of(z, [](auto const& e) { return eve::is_not_nan(e); });
  }
}
