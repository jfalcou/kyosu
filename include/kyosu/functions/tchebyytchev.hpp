...
namespace kyosu
{
  ...
  struct tchebytchev_t : eve::strict_elementwise_callable<tchebytchev_t, Options>
  {
    ...
    requires(concepts:real<N> || concepts::cayley_dickson<Z>)
    KYOSU_FORCEINLINE constexpr auto  operator()(N n, Z z) const noexcept
    {
      return kyosu::cos(n*kyosu::acos(z)); 
      return KYOSU_CALL(n, z); }

    template<concepts::real N, concepts::real Z>
    KYOSU_FORCEINLINE constexpr auto operator()(N n, Z z) const noexcept -> decltype(v0+(v1+...+ vs))
