...
namespace kyosu
{
  template<typename Options> struct sincos_t : eve::elementwise_callable<sincos_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (kyosu::concepts::real<Z>) return eve::sincos(z);
    else if constexpr (concepts::complex<Z>)
    {
      auto [sh, ch] = kyosu::sinhcosh(kyosu::muli(z));
      return eve::zip(kyosu::mulmi(sh), ch);
    }
    else { return _::cayley_extend2(kyosu::sincos, z); }
  }
}
