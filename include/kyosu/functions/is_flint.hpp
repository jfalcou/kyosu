...
namespace kyosu
{
  template<typename Options> struct is_flint_t : eve::elementwise_callable<is_flint_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::is_flint(z);
    else return kyosu::is_real(z) && eve::is_flint(real(z));
  }
}
