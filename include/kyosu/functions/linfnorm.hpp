...
namespace kyosu
{
  ...
  struct linfnorm_t
    : eve::strict_elementwise_callable<linfnorm_t, Options, raw_option, pedantic_option, kyosu::flat_option>
  {
    template<concepts::cayley_dickson_like Z1, concepts::cayley_dickson_like... Zs>
    requires(eve::same_lanes_or_scalar<Z1, Zs...>)
    KYOSU_FORCEINLINE constexpr eve::common_type_t<as_real_type_t<Z1>, as_real_type_t<Zs>...> operator()(
      Z1 const& z1, Zs const&... zs) const noexcept
    {
...
namespace kyosu::_
{
  ...
  EVE_FORCEINLINE constexpr auto linfnorm_(KYOSU_DELAY(), O const&, Z1 const& z1, Zs const&... zs) noexcept
  {
    ...
    {
      if constexpr (sizeof...(zs) == 0) return eve::abs(z1);
      else return eve::maxabs(z1, zs...);
    }
    else if constexpr (O::contains(kyosu::flat))
    {
      if constexpr (sizeof...(zs) == 0) return eve::maxabs(kyosu::real(z1), kyosu::imag(z1));
      else
    ...
    else
    {
      ...
      else return kyosu::maxabs(z1, zs...);
    }
  }
}
