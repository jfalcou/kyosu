...
namespace kyosu
{
  template<typename Options> struct is_not_finite_t : eve::elementwise_callable<is_not_finite_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::is_not_finite(z);
    else return kumi::any_of(z, [](auto const& e) { return eve::is_not_finite(e); });
  }
}
