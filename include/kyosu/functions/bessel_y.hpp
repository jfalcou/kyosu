...
namespace kyosu
{
  ...
  struct bessel_y_t
    : eve::strict_elementwise_callable<bessel_y_t, Options, eve::spherical_option, eve::cylindrical_option>
  {
    ...
    KYOSU_FORCEINLINE constexpr Z operator()(N const& n, Z const& z, std::span<Z, S> ys) const noexcept
    {
  ...
  //!   namespace kyosu
  //!   {
  //!      template<eve;scalar_value N, kyosu::concepts::cayley_dickson T>    constexpr auto bessel_y(N n, T z)
  //!      noexcept; //1 template<eve;scalar_value N, kyosu::concepts::complex T, size_t S> constexpr auto bessel_y(N n,
  //!      T z, std::span<Z, S> cys)            noexcept; //2 template<eve;scalar_value N,
  //!      kyosu::concepts::cayley_dickson T>    constexpr auto bessel_y[spherical](N n, T z) noexcept; //3
  //!      template<eve;scalar_value N, kyosu::concepts::complex T, size_t S> constexpr auto bessel_y[spherical](N n, T
  //!      z, std::span<Z, S> sys) noexcept; //4
  //!   }
  //!   @endcode
  //!
  //!   **Parameters**
  ...
  //!   *  [Wolfram MathWorld: Bessel Function of the Second
  //!   Kind](https://mathworld.wolfram.com/BesselFunctionoftheSecondKind.html)
  //!   *  [Wolfram MathWorld: Spherical Bessel Function of the Second
  ...
  KYOSU_FORCEINLINE constexpr auto bessel_y_(KYOSU_DELAY(), O const& o, N n, Z z, YS& ys) noexcept
  {
    if constexpr (eve::integral_scalar_value<N>)
    {
      if constexpr (O::contains(eve::spherical))
      {
        ...
        else return _::sb_yn(n, z, ys);
      }
      else return _::cb_yn(n, z, ys);
    }
    else
    {
      if constexpr (O::contains(eve::spherical)) return _::sb_yr(n, z, ys);
      else return _::cb_yr(n, z, ys);
    }
  }
}
