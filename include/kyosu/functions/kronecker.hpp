...
template<typename Options>
  struct kronecker_t : eve::strict_tuple_callable<kronecker_t, Options>
  {
...
//!      constexpr auto kronecker(kumi::non_empty_product_type auto const& tup)             noexcept; // 2
//!
//!   }
//!   @endcode
//!
...
namespace kyosu::_
{
  ...
  EVE_FORCEINLINE constexpr auto kronecker_(KYOSU_DELAY(), O const&, T0 const & t0, T1 const & t1, Ts const&... ts) noexcept
  {
    using r_t =kyosu::as_real_type_t< kyosu::as_cayley_dickson_like_t<T0, T1, Ts...>>;
    if constexpr(sizeof...(Ts) == 0)
      return kyosu::if_else(is_equal(t0, t1), kyosu::one(eve::as<r_t>()), zero);
    else
    {
