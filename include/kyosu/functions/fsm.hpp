...
namespace kyosu
{
  template<typename Options> struct fsm_t : eve::strict_elementwise_callable<fsm_t, Options, raw_option, pedantic_option>
  {
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto fsm_(KYOSU_DELAY(), O const&, Z0 z0, Z1 z1, Z2 z2) noexcept
  {
    if constexpr (concepts::real<Z0> && concepts::real<Z1> && concepts::real<Z2>) return eve::fsm(z0, z1, z2);
    else return z1 * z2 - z0;
  }
}
