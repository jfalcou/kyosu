...
namespace kyosu
{
  template<typename Options> struct faddeeva_t : eve::elementwise_callable<faddeeva_t, Options, raw_option, pedantic_option>
  {
  ...
  KYOSU_FORCEINLINE constexpr auto faddeeva_(KYOSU_DELAY(), O const&, Z z) noexcept
  {
    if constexpr (concepts::complex<Z>)
    {
      ...
      auto fexp = [iosqrtpi, sqrtpi](auto z) { //Fourier expansion approximation
        ...
        constexpr std::array<real_t, maxN> aN = {
          //Fourier coefficients
          2.758402332921771e-01, 2.245739552246158e-01, 1.594149382739117e-01, 9.866576641545419e-02,
          5.324414078763941e-02, 2.505215000539365e-02, 1.027746567053954e-02, 3.676164332844847e-03,
          1.146493641242233e-03, 3.117570150461976e-04, 7.391433429603010e-05, 1.527949342800837e-05,
          2.753956608221073e-06, 4.327858781901246e-07, 5.930030408745884e-08, 7.084490307748205e-09,
          7.379520635816785e-10, 6.702171606002010e-11, 5.307265163470805e-12, 3.664324113467642e-13,
      ...
      auto contfr = [iosqrtpi](auto z) { // the Laplace continued fraction approximation
        constexpr std::array<real_t, 11> bN = {5.5000, 5.0000, 4.5000, 4.0000, 3.5000, 3.0000,
      ...
      auto small_z = [sqrtpi](auto z) {
        ...
        return (((6 - 6 * zP2 + 3 * zP4 - zP6) *
                 (15 * sqrtpi + complex(real_t(0), real_t(1)) * z * (30 + 10 * zP2 + 3 * zP4))) /
                (90 * sqrtpi));
      };
      ...
      auto narr_band = [sqrtpi](auto z, auto aN, auto tauM) { // the narrow band
        ...
        Z NB{};
        for (size_t n = 1; n <= maxN; ++n)
          NB += (aN[n - 1] * dec(eve::sign_alternate(real_t(n)) * z1) / (sqr(n * eve::pi(eve::as(tauM))) - z2));
      ...
      auto smallim = [narr_band, small_z](auto z) { // approximation at small imag(z)
        constexpr size_t maxN = 23;
        constexpr std::array<real_t, maxN> aN12 = {
          //Fourier coefficients
          2.758402332921771e-01, 2.245739552246158e-01, 1.594149382739117e-01, 9.866576641545419e-02,
          5.324414078763941e-02, 2.505215000539365e-02, 1.027746567053954e-02, 3.676164332844847e-03,
          1.146493641242233e-03, 3.117570150461976e-04, 7.391433429603010e-05, 1.527949342800837e-05,
          ...
          7.379520635816785e-10, 6.702171606002010e-11, 5.307265163470805e-12, 3.664324113467642e-13,
          2.205894944941035e-14, 1.157826862628556e-15, 5.298711429467307e-17};
        constexpr std::array<real_t, maxN> aN12_1 = {
          //Fourier coefficients avoiding poles
          2.738693653257005e-01, 2.237253191645656e-01, 1.597109174949294e-01, 9.963269094255395e-02,
          5.431463971403691e-02, 2.587496275171680e-02, 1.077185133049561e-02, 3.918760804860615e-03,
          1.245818993506881e-03, 3.461058398397597e-04, 8.402542038611651e-05, 1.782626047239582e-05,
          3.304894416742214e-06, 5.354300211460283e-07, 7.580461285556943e-08, 9.378563805151443e-09,
          1.013969351293613e-09, 9.579902872635222e-11, 7.909429715194891e-12, 5.706594634502440e-13,
        ...
        return if_else(!ind_0, if_else(ind_poles, narr_band(z, aN12_1, real_t(12.1)), narr_band(z, aN12, real_t(12))),
                       small_z(z));
      };
