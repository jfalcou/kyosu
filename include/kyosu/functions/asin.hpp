...
namespace kyosu
{
  ...
  struct asin_t : eve::strict_elementwise_callable<asin_t, Options, real_only_option, rad_option, radpi_option>
  {
...
namespace kyosu
{
  namespace _
  {
    ...
    {
      if constexpr (O::contains(radpi)) return radinpi(kyosu::asin[o.drop(radpi)](a0));
      else if constexpr (O::contains(real_only) && concepts::real<Z>) return eve::asin(a0);
      ...
      else if constexpr (concepts::complex<Z>)
      {
        ...
        rtype apx = a + x;
        r =
          eve::if_else(lexone, eve::atan(x / eve::sqrt(eve::half(eve::as(a0r)) * apx * (yy / (tr + xp1) + (ts - xm1)))),
    ...
    requires(!O::contains(real_only))
    {
      ...
      return kyosu::asin[o](z) + branch_correction<O>(kk);
    }
    ...
    requires(!O::contains(real_only))
    {
      return eve::detail::mask_op(cx, eve::detail::return_2nd, complex(z), asin[o](z, k...));
    }
  }

  inline constexpr auto asinpi = asin[radpi];
