...
namespace kyosu
{
  template<typename Options> struct sech_t : eve::elementwise_callable<sech_t, Options, raw_option, pedantic_option>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (kyosu::concepts::real<Z>) return eve::sech(z);
    else return kyosu::rec(kyosu::cosh(z));
  }
}
