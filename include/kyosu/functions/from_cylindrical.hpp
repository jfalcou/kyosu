...
namespace kyosu
{
  ...
  struct from_cylindrical_t : eve::elementwise_callable<from_cylindrical_t, Options>
  {
    ...
    KYOSU_FORCEINLINE constexpr auto operator()(V const& r, U const & angle
                                               , W const & h1
                                               , T const & h2) const noexcept -> quaternion_t<decltype(r+angle+h1+h2)>
    {
      using e_t = decltype(r+angle+h1+h2);
      auto [sa, ca] = eve::sincos(angle);
      return kyosu::quaternion(r*ca, r*sa, h1, h2);

//      return KYOSU_CALL(r, angle, h1, h2);
    }
