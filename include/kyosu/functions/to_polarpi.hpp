...
namespace kyosu
{
  template<typename Options> struct to_polarpi_t : eve::elementwise_callable<to_polarpi_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (kyosu::concepts::complex<C>) return kumi::tuple{kyosu::abs(c), kyosu::argpi(c)};
    else return kumi::tuple{kyosu::abs(c), kyosu::argpi(c), sign(ipart(c)) * sign(pure(c))};
  }
}
