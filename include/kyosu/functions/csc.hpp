...
namespace kyosu
{
  template<typename Options> struct csc_t : eve::elementwise_callable<csc_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::csc(z);
    else if constexpr (concepts::complex<Z>) return kyosu::rec(kyosu::sin(z));
    else return _::cayley_extend(kyosu::csc, z);
  }
}
