...
namespace kyosu
{
  ...
  struct div_t : kyosu::strict_tuple_callable<div_t, Options, raw_option, pedantic_option, eve::kahan_option>
  {
  ...
  //!
  //! @note If all elements are real typed the result will be real typed, using a call to `eve::div`
  //!
  //!  @groupheader{Example}
  //!  @godbolt{doc/div.cpp}
  //======================================================================================================================
...
namespace kyosu::_
{
  ...
  EVE_FORCEINLINE constexpr auto div_(KYOSU_DELAY(), O const& o, T0 const& t0, Ts const&... ts) noexcept
  {
    ...
    if constexpr (concepts::real<r_t>) return eve::div(t0, ts...);
