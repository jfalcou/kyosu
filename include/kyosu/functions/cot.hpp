...
namespace kyosu
{
  template<typename Options> struct cot_t : eve::elementwise_callable<cot_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::cot(z);
    else if constexpr (concepts::complex<Z>) return kyosu::rec(kyosu::tan(z));
    else return _::cayley_extend(kyosu::cot, z);
  }
}
