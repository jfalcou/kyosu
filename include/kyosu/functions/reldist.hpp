...
namespace kyosu
{
  ...
  struct reldist_t
    : eve::strict_elementwise_callable<reldist_t, Options, raw_option, eve::pedantic_option, eve::numeric_option>
  {
...
namespace kyosu::_
{
  ...
  constexpr auto reldist_(KYOSU_DELAY(), O const& o, C0 c0, C1 c1) noexcept
  {
    ...
    return if_else(is_infinite(d) || is_eqz(d), d,
                   d / eve::max(kyosu::abs(c0), kyosu::abs(c1), eve::one(eve::as(abs(c0)))));
  }
}
