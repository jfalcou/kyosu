...
namespace kyosu
{
  ...
  struct hypergeometric_t
    : eve::
        strict_elementwise_callable<hypergeometric_t, Options, raw_option, pedantic_option, kyosu::regularized_option>
  {
    ...
    template<typename T1, typename T2, typename T3> using res_t = typename res<T1, T2, T3>::type;

    template<concepts::complex_like Z, eve::product_type T1, eve::product_type T2>
    constexpr KYOSU_FORCEINLINE res_t<Z, T1, T2> operator()(Z z, T1 a, T2 b) const noexcept
    {
      if constexpr (!Options::contains(kyosu::regularized) || T2::empty())
      {
        if constexpr (concepts::real<Z>) return _::hyperg(kyosu::complex(z), a, b);
        else return _::hyperg(z, a, b);
      }
      else
      {
        if constexpr (concepts::real<Z>) return _::hyperg(kyosu::complex(z), a, b, kyosu::regularized);
        else return _::hyperg(z, a, b, kyosu::regularized);
      }
    }
  };
