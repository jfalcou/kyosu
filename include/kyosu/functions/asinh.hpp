...
namespace kyosu
{
  template<typename Options> struct asinh_t : eve::elementwise_callable<asinh_t, Options>
  {
  ...
  //!      * If z is \f$ \textrm{NaN}+ iy\f$ (for any finite nonzero y), the result is \f$ \textrm{NaN}+ i
  //!      \textrm{NaN}\f$
  //!      * If z is \f$ \textrm{NaN}+i \infty\f$, the result is \f$\pm \infty+ i \textrm{NaN}\f$ (the sign of the real
  //!      part is unspecified)
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::asinh(z);
    if constexpr (concepts::complex<Z>) return mulmi(kyosu::asin(muli(z)));
    else return kyosu::_::cayley_extend(kyosu::asinh, z);
  }
}
