...
namespace kyosu
{
  template<typename Options> struct cscpi_t : eve::elementwise_callable<cscpi_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::cscpi(z);
    else if constexpr (concepts::complex<Z>) return kyosu::rec(kyosu::sinpi(z));
    else return _::cayley_extend(kyosu::cscpi, z);
  }
}
