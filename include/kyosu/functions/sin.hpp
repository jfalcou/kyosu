...
namespace kyosu
{
  template<typename Options> struct sin_t : eve::elementwise_callable<sin_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (kyosu::concepts::real<Z>) return eve::sin(z);
    else if constexpr (concepts::complex<Z>) return kyosu::muli(kyosu::sinh(Z(kyosu::mulmi(z))));
    else return _::cayley_extend(kyosu::sin, z);
  }
}
