...
namespace kyosu
{
  ...
  struct deta_t : eve::strict_elementwise_callable<deta_t, Options, raw_option, pedantic_option>
  {
    ...
    {
      if constexpr (concepts::real<Z>) return (*this)(k, complex(z));
      else return KYOSU_CALL(k, z);
    }
...
namespace kyosu::_
{
  ...
  constexpr auto deta_(KYOSU_DELAY(), O const&, K const& kk, Z z) noexcept
  {
    if constexpr (concepts::complex<Z>)
    {
      ...
      constexpr std::array<real_t, 64> cm = {.27105054312137610850e-19 - .17889335846010823161e-17,
