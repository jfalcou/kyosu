...
namespace kyosu
{
  template<typename Options> struct expm1_t : eve::elementwise_callable<expm1_t, Options>
  {
    template<concepts::cayley_dickson_like Z> KYOSU_FORCEINLINE constexpr Z operator()(Z const& z) const noexcept
    {
      if constexpr (concepts::real<Z>) return eve::expm1(z);
      else if constexpr (kyosu::concepts::complex<Z>)
      {
        auto [rz, iz] = z;
        auto [siz, ciz] = eve::sincos(iz);
        auto cosm1 = [siz, ciz](auto b) {
          return eve::if_else(abs(ciz) < eve::pio_4(eve::as(b)), eve::dec(ciz), -eve::sqr(siz) / (eve::inc(ciz)));
        };
      ...
      else { return _::cayley_extend(*this, z); }
    }
