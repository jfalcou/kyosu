...
namespace kyosu
{
  ...
  struct is_equal_t
    : eve::strict_elementwise_callable<is_equal_t, Options, raw_option, pedantic_option, eve::numeric_option>
  {
...
namespace kyosu::_
{
  ...
  KYOSU_FORCEINLINE constexpr auto is_equal_(KYOSU_DELAY(), O const& opts, Z0 z0, Z1 z1) noexcept
  {
    if constexpr (O::contains(eve::numeric))
    {
      ...
      else { return kumi::all_of(kumi::map(eve::is_equal[eve::numeric], z0, z1)); }
    }
    else return z0 == z1;
  }
}
