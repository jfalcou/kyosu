...
namespace kyosu
{
  template<typename Options> struct tanpi_t : eve::elementwise_callable<tanpi_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (concepts::real<Z>) return eve::tanpi(z);
    ...
    else { return _::cayley_extend(kyosu::tanpi, z); }
  }
}
