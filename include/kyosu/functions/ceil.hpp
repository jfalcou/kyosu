...
namespace kyosu
{
  template<typename Options> struct ceil_t : eve::elementwise_callable<ceil_t, Options>
  {
...
namespace kyosu::_
{
  ...
  {
    if constexpr (kyosu::concepts::real<Z>) return eve::ceil(z);
    else return Z{kumi::map([](auto const& e) { return eve::ceil(e); }, z)};
  }
}
