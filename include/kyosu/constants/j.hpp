...
namespace kyosu
{
  template<typename Options> struct j_t : eve::constant_callable<j_t, Options>
  {
    template<typename T>
    struct result
      : std::conditional<(concepts::cayley_dickson<T> && dimension_v<T> > 2), T, as_cayley_dickson_n_t<4, T>>
    {
    };
