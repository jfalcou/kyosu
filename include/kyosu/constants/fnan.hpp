...
namespace kyosu
{
  template<typename Options> struct fnan_t : eve::constant_callable<fnan_t, Options>
  {
    template<typename T> struct result : std::conditional<concepts::cayley_dickson<T>, T, as_cayley_dickson_n_t<2, T>>
    {
    };

    template<typename T> static KYOSU_FORCEINLINE constexpr auto value(eve::as<T> const&, auto const&)
    {
    ...
    template<concepts::real T> KYOSU_FORCEINLINE constexpr T operator()(as<T> const& v) const
    {
      return eve::nan(as(v));
    }
